...
const Cart = () => {
  ...
  const { products, currency , token } = useSelector((state) => state.products);
  ...
  const deleteProduct =  async (id, size) => {
    ...
    try {
      const response  = await axios.post(backendUrl + '/api/cart/delete' , {id , size} , {headers : {token}} )
      if(response.data.success){
        toast.success(response.data.message)
        dispatch(initializeCart(response.data.cartData))
      }
      else{
  ...
  const updateCartQuantity = async (size, id, type, quantity) => {

    try {
      const response = await axios.post(backendUrl + '/api/cart/update'  , {size , id , type} , {headers : {token}} )
      if(response.data.success){
        dispatch(initializeCart(response.data.cartData))
      }else{
        toast.error(response.data.message)
